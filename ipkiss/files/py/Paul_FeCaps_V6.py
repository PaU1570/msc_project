...
my_PETs_20CAP = Structure(name="FeCap_20")
...
for i in range(numberOfDevices):
    ...
    for FESize in FE_Contact_Sizes:
        ...
        for island_value in islandCcl_values:
            full_textLabel = 'ID%i %.0f' % (deviceNbr, FESize)
            textLabel = 'ID%i' % (deviceNbr)
            output_data.append([deviceNbr, "FeCap_20", '%.0f' % (FESize * 1000)]) 
            temp1 = FeCAP(FEcontactSize = FESize / 2.0,  # radius
                           MOcontactSize = 5.0,
                           viaDistance = 10.0,
                           islandCcl = island_value,
            ...
            if deviceNbr not in [1032,1033,1000,1001,968,969]:
                my_PETs_20CAP += SRef(temp1, position=(curX, curY))           
            curX += dX
            deviceNbr += 1
    curY += dY
    nY += 1           
...
my_layout += SRef(my_PETs_20CAP, position = Coord2(-4700, -635.00))
