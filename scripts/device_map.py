...
MODE = 2 # 0: Individual plots; 1: Combined plot, 2: Yield plot
# Get map files
...
for f in csv_files:
    ...
    if match:
        ...
        if MODE == 0:
            ...
            idata.append([x_coord, y_coord, df['Iabs'].max()])
            continue
...
if MODE == 2:
    ...
    im = ax.scatter(idata[:,0], idata[:,1], c=idata[:,2], cmap='plasma', s=100)
    fig.colorbar(im, label='Max current (A)')
    plt.tight_layout()
