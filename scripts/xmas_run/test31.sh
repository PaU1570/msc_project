...
ANALYSIS_SCRIPT_SOURCE="${PARENT_DIR}/aihwkit/run_mnist_mp.sh"
if [ $# -ne 2 ]; then
...
write_noise_std_vals=($(seq 0 0.05 0.5))
for write_noise_std in "${write_noise_std_vals[@]}"; do
    ((i=i%N_BATCH)); ((i++==0)) && wait
    bash "$ANALYSIS_SCRIPT_SOURCE" "$data_directory" "$output_directory/write_noise_std_${write_noise_std}" 25 0 0 "$write_noise_std" 0 0 &
done
