...
for file in $files; do
    ...
    if ! python ${ANALYSIS_SCRIPT_SOURCE} $file --output_dir ${output_directory} --epochs ${epochs} --dw_min_dtod ${dw_min_dtod} --dw_min_std ${dw_min_std} --write_noise_std_mult ${write_noise_std_mult} --save_weights --use_reference_device ; then
