...
ANALYSIS_SCRIPT_SOURCE="${SCRIPT_DIR}/mnist_mixedprecision_nomod.py"

if [ $# -lt 3 ]; then
    echo "Usage: $0 <analyzed data directory> <output directory> [epochs (25)] [dw_min_dtod (0.3)] [dw_min_std (0.3)] [write_noise_std (0.0)] [w_min_dtod (0.3)] [w_max_dtod (0.3)] [granularity (0)] [batch size (64)]"
    exit 1
fi
